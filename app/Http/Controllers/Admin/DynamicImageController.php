    ...
    public function generateImage(Request $request)
    {
        try {
            ...
            $text            = $request->input('text',
                '根据The Block的报道，NFT市场监测数据显示，CryptoPunks系列上周交易量突破2460万美元，创下自2024年3月以来的单周最高纪录，较前一周激增416%。市场的活跃度伴随着价格指标全面上涨：底价从40 ETH攀升至47.5 ETH，平均售价从14万美元上涨至18.2万美元。这一行情复苏与纳斯达克上市公司GameSquare的里程碑式收购密切相关，该公司近期发行价值515万美元的优先股，以三倍于市场底价的估值购入稀有"Ape Punk"系列中的#5577号作品。');
            $fontSize        = 14;
            $textColor       = $request->input('text_color', '#000000');
            $backgroundColor = $request->input('bg_color', '#FDF1E5');
            $quality         = (int)$request->input('quality', 90);  // 图片质量 0-100
            $scale           = (float)$request->input('scale', 1);   // 高分辨率缩放倍数

            $title         = $request->input('title', '美股BMNR涨幅扩至126.34%，现报价129.86美元');
            $titleFontSize = 18;
            ...
            return response($imageData)
                ...
                ->header('Content-Disposition', 'inline; filename="generated-image.' . $format . '"');
